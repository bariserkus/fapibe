...
def test_read_all_authenticated(test_todo):
    ...
    assert response.json() == [{
        "complete": False,
        "title": "Learn to Code",
        "description": "Need to Learn Python",
        "priority": 5,
        "id": 1,
        "owner_id": 1,}]
...
def test_update_todo(test_todo):
    ...
    respose = client.put("/todos/todo/1", json=request_data)
...
def test_update_todo_not_found(test_todo):
    ...
    respose = client.put("/todos/todo/999", json=request_data)
    assert respose.status_code == status.HTTP_404_NOT_FOUND
    assert respose.json() == {"detail": "Todo Not Found!"}
